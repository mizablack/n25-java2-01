...
public class Duke extends Group {
	...
	public Duke() {dukeImage = new Image(getClass().getResource("Images/Duke.png").toString());
					...
					dukeImgVw.setPreserveRatio(true);// mater proporção da imagem
					dukeImgVw.setY(10);
					
					luvaImgVw.setFitWidth(38);
	...
	private void criacaoEventos() {
		...
		setOnMouseClicked((me)-> { double largura = this.getBoundsInLocal().getWidth() / 2;
								double altura = this.getBoundsInLocal().getHeight() / 2;
								// posicionamento o personagem na scena
